...
DATA_PATH = r"intermediate_combined_data.csv"
        ...
        try:
            ...
            if len(selected) > 0:
                ...
                try:
                    ...
                    st.markdown("#### 📊 Feature Change Magnitude")
                    delta = (adversarial_filled - sample_filled).abs().T
                    delta.columns = ['Change']
                    bar = px.bar(delta, y='Change', labels={'index': 'Feature', 'Change': 'Magnitude'}, title='Feature-wise Change Magnitude')
                    st.plotly_chart(bar, use_container_width=True)

                    st.markdown("#### 🔍 PCA Projection of Perturbation")
                    try:
                        pca = PCA(n_components=2)
                        combined = pd.concat([sample_filled, adversarial_filled])
                        projected = pca.fit_transform(combined)
                        pca_df = pd.DataFrame(projected, columns=['PC1', 'PC2'])
                        pca_df['Type'] = ['Original', 'Adversarial']
                        fig_pca = px.scatter(pca_df, x='PC1', y='PC2', color='Type', title='2D PCA of Sample Shift')
                        st.plotly_chart(fig_pca, use_container_width=True)
                    except:
                        st.warning("PCA projection failed due to feature mismatch.")

                    st.markdown("#### Feature Values & Export")
